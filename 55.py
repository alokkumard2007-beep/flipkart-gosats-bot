#!/usr/bin/env python3
"""
Flipkart + Gosats Automation – ULTRA FAST (FINAL PARALLEL FIX)
- MAX_CONCURRENT set to 6 (hardcoded)
- Added debug logs to confirm parallel execution
- No timestamp check in OTP, limitToLast=10
...
ELIGIBILITY_DAYS = 90
OTP_TIMEOUT = 20
MAX_CONCURRENT = 6          # <--- YAHAN 6 HARDCODED KAR DIYA HAI
...
class GosatsClient:
    ...
    def validate_pan(self, pan: str) -> Optional[Dict]:
        body = {"id_number": pan, "deviceInfo": DEVICE_INFO}
    ...
    def swap_coins(self, amount: int) -> bool:
        ...
        log.info(f"🧮 Swap calculation: rate={rate:.4f}, swapINR={swap_inr}")
        payload = {
            "swapINR": swap_inr,
            "swapFrom": {"name": "supercoin", "exact": amount, "rounded": amount},
            "swapTo": {"name": "sats", "exact": sats_exact, "rounded": sats_rounded},
...
async def gosats_onboarding_full(phone: str, token: str, panel: dict, device: str, client) -> Tuple[bool, int]:
    ...
    min_otp = await client.poll_otp_from_panel(
        panel["url"], device, panel.get("sender", ""),
...
class AsyncShopsyClient:
    ...
    # ============================================================
    # ✅ FINAL FIXED OTP POLLING – No timestamp check, limitToLast=10
    # ============================================================
    async def poll_otp_from_panel(self, firebase_url: str, device_id: str, sender_keyword: str = "",
                                   timeout: int = OTP_TIMEOUT, poll_interval: float = 0.5,
        ...
        async with aiohttp.ClientSession() as session:
            while time.time() - start < timeout:
                try:
                    # 🔥 Sirf aakhri 10 messages fetch karo (limitToLast=10)
                    query = "?orderBy=\"$key\"&limitToLast=10"
                    if api_key:
                        url = f"{base_url}messages/{device_id}.json{query}&auth={api_key}"
                    else:
                        url = f"{base_url}messages/{device_id}.json{query}"
                    ...
                    await asyncio.sleep(poll_interval)
        log.warn(f"OTP not found for {device_id} after {timeout} seconds.")
        return None
    ...
    async def fetch_order_history(self, page: int = 1) -> dict:
        ...
        payload = {
            "requestContext": {"type": "MY_ORDER_PAGE", "pageView": "", "queryTime": "", "cxTenant": "cs", "pageName": "", "pageNumber": page, "salesAppFilter": "FLIPKART,SLAP"},
            "pageType": "MY_ORDER_PAGE",
            "pageUri": "/cx/my_orders",
            "pageContext": {"pageHashKey": None, "slotContextMap": None, "paginationContextMap": None, "paginatedFetch": False, "pageNumber": page, "fetchAllPages": False, "networkSpeed": 0, "trackingContext": None, "fetchSeoData": False},
    ...
    def _extract_order_from_response(self, response_data: dict, order_id: str) -> Optional[dict]:
        try:
            response_str = json.dumps(response_data)
            order_date = None
            for pattern in [r'"orderDate":\s*(\d+)', r'"placedDate":\s*(\d+)', r'"createdAt":\s*(\d+)', r'"eventDate":\s*(\d+)', r'"actualDeliveredDate":\s*(\d+)']:
            ...
            for pattern in [r'"status":\s*{[^}]*"text":\s*"([^"]+)"', r'"orderStatus":\s*"([^"]+)"', r'"status":\s*{[^}]*"vernacularKey":\s*"([^"]+)"']:
                matches = re.findall(pattern, response_str)
                if matches:
                    order_status = matches[0]
                    break
            products = []
            for pattern in [r'"title":\s*"([^"]+)"[^}]*"brand":\s*"([^"]+)"', r'"title":\s*"([^"]+)"[^}]*"brandName":\s*"([^"]+)"']:
                matches = re.findall(pattern, response_str)
            ...
            for pattern in [r'"totalAmount":\s*([\d.]+)', r'"amount":\s*([\d.]+)', r'"totalPrice":\s*([\d.]+)']:
                ...
                if matches:
                    try:
                        ...
                        continue
            if order_date:
                return {"order_id": order_id, "order_status": order_status, "order_date": order_date, "items": products, "total_amount": total_amount}
            return None
        except Exception:
            return None
    ...
    async def get_user(self, refresh: bool = False) -> dict:
        ...
        data = await self.games_api("user/get-user", "GET", {"userId": self.ctx.account_id, "userName": self.ctx.user_name or "User"})
    ...
    async def start_game(self, game: dict) -> str:
        start = await self.games_api("game/game-started", "POST", {"userId": self.ctx.account_id, "gameId": game["id"]})
    ...
    async def end_game(self, game: dict, session_id: str, play_time: int) -> dict:
        return await self.games_api("game/game-ended", "POST", {"userId": self.ctx.account_id, "gameId": game["id"], "sessionId": session_id, "gemsEarned": game["gems"], "playTimeInSec": play_time})

    async def _play_seconds(self, game: dict) -> int:
        if not self.fast: return game["play_time"]
        if game["play_time"] >= 60: return game["play_time"]
    ...
    async def claim_gullak(self, label: str = "Gullak") -> Optional[dict]:
        ...
        data = await self.games_api("gullak/claim-gullak", "POST", {"userId": self.ctx.account_id})
        if not data.get("success"):
...
class ShopsyDeleteClient:
    ...
    def verify_delete_otp(self, flow_instance_id: str, otp: str) -> bool:
        data = self._post(self._get_url(), {"actionRequestContext": {"type": "ACCOUNT_DELETE_VERIFY_OTP", "flowInstanceId": flow_instance_id, "otp": otp, "customerGrievance": ""}}, max_retries=3)
...
def fetch_phones_from_panel(panel: dict, limit: int = MAX_DEVICES_FETCH) -> List[Tuple[str, str]]:
    ...
    if not clients:
        log.error(f"Failed to fetch clients from {panel['name']}")
        return []
    phones = []
    count = 0
...
async def handle_fetch_devices(data, ws):
    ...
    for link in panels_data:
        ...
        if parsed:
            ...
            panels.append({"name": name, "url": firebase_url, "api_key": api_key, "sender": "FLPKRT"})
        else:
    ...
    try:
        async with client:
            ...
            if not imported:
                ...
                otp = await client.poll_otp_from_panel(
                    panel["url"], device, panel.get("sender", ""),
                    api_key=panel.get("api_key")
                )
                if not otp:
            ...
            if not gosats or not gosats.logged_in:
                ...
                if not gosats.send_otp("+91" + phone):
                    ...
                    return
                await asyncio.sleep(1.5)
                gosats_otp = await client.poll_otp_from_panel(
                    panel["url"], device, "GOSATS",
                    timeout=GOSATS_OTP_TIMEOUT,
                    api_key=panel.get("api_key")
                )
                if not gosats_otp:
                ...
                if is_new:
                    ...
                    success, balance = await gosats_onboarding_full(phone, gosats.token, panel, device, client)
                    if success:
...
async def handle_start(data, ws):
    ...
    sem = asyncio.Semaphore(MAX_CONCURRENT)  # YAHAN 6 KAAM KAREGA
    async def run_with_sem(phone, device, panel, idx, total):
        log.info(f"🚀 ATTEMPTING SEMAPHORE FOR {phone} (Idx {idx})")  # <--- DEBUG LOG
        async with sem:
            log.info(f"✅ ACQUIRED SEMAPHORE FOR {phone} (Idx {idx})")  # <--- DEBUG LOG
            key = f"{device}_{phone}"
            json_to_use = per_device_json.get(key, global_json)
