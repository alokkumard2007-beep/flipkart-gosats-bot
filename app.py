...
- JSON Login support (import session without OTP) - ONLY when explicitly provided
...
# ===================== SHOPSY DELETE CLIENT (from 555.py) =====================
        ...
        async with client:
            ...
            if json_data:
                if client.import_session(json_data):
                    ...
                    # DO NOT save session file when using JSON import
            
            # NEVER load from file - always do OTP login if no JSON import
            # This ensures every run uses fresh OTP login unless explicitly using JSON
            
            # Always fallback to OTP login (even if JSON import failed)
            if not imported:
                ...
                # DO NOT save session - no file creation
                # Session tokens are only kept in memory for this run
            ...
            if not gosats or not gosats.logged_in:
                ...
                if not gosats.send_otp("+91" + phone):
                    ...
                    return
                gosats_otp = await client.poll_otp_from_panel(panel["url"], device, "GOSATS", timeout=GOSATS_OTP_TIMEOUT)
                if not gosats_otp:
                ...
                if is_new:
                    ...
                    # Activate SuperCoins if needed - uses correct endpoint
                    config = gosats.get_config()
                    if not config or config.get("data", {}).get("swapConfig") is None:
            ...
            if balance > 0:
                ...
                if gosats.swap_coins(balance):
                    ...
                    await emit_to_all('status', {'device': device_key, 'status': 'error', 'message': 'Swap failed'})
                    return
            else:
            ...
            await emit_to_all('await_input', {'device': device_key, 'type': 'confirm_swap', 'message': f'Confirm swap for {phone} (coins: {total_coins})', 'coins': total_coins})
            ...
            # ----- Delete Flipkart account (from 555.py) -----
            ...
            if not flow_id:
                ...
                return
            delete_otp = await client.poll_otp_from_panel(panel["url"], device, panel["sender"])
            if not delete_otp:
                await emit_to_all('status', {'device': device_key, 'status': 'error', 'message': 'Delete OTP not received from panel'})
                return
            if del_client.verify_delete_otp(flow_id, delete_otp):
                # NO session file deletion since we don't save sessions
...
async def handle_start(data, ws):
    ...
    async def run_with_sem(phone, device, panel, idx, total):
        async with sem:
            await process_one(phone, device, panel, idx, total, is_single=False, json_data=json_data)
...
async def handle_start_single(data, ws):
    ...
    task = asyncio.create_task(process_one(phone, device, panel, 1, 1, is_single=True, json_data=json_data))
    active_tasks.append(task)
    task.add_done_callback(lambda t: active_tasks.remove(t) if t in active_tasks else None)
